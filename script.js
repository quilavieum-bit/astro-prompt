...
const MOON_PHASES = [
  ...
  { date: '2026-05-06', time: '02:00', type: '新月', symbol: '🌑', desc: '牡牛座15°' },
  { date: '2026-05-20', time: '18:00', type: '満月', symbol: '🌕', desc: '蠍座0°' },
  ...
  { date: '2026-05-13', time: '02:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-05-28', time: '15:00', type: '下弦', symbol: '🌗', desc: '' },
...
function getLatestMoonPhase(dateStr) {
  const past = MOON_PHASES.filter(mp => mp.date <= dateStr && (mp.type === '新月' || mp.type === '満月'));
...
function generateMode1(dateStr) {
  ...
  const todayPhase = MOON_PHASES.find(mp => mp.date === dateStr);
  ...
  const vibe = moonVibes[moon.sign] || 'ひとつひとつが丁寧な';
  
  // タイトアスペクト（orb1.5°以内）から最重要情報
  const tightAsp = aspects.filter(a => a.aspect.tight).slice(0,2);
  ...
  let phaseNote = '';
  if (todayPhase) {
    if (todayPhase.type === '新月') phaseNote = `今日は${todayPhase.type}。新しいページをめくるような日かも。\n\n`;
    else if (todayPhase.type === '満月') phaseNote = `今日は${todayPhase.type}。何かが完成に近づいているのかもしれない。\n\n`;
  }
  ...
  if (ingresses.length > 0) {
    ...
    ingressNote = `${ing.planet}が${ing.sign}へ。空気が少し変わった感じがするかもしれない。\n\n`;
  }
  
  // タイトアスペクトから雰囲気
  let aspNote = '';
  if (tightAsp.length > 0) {
    const a = tightAsp[0];
    if (a.aspect.symbol === '☌') aspNote = `${a.transit}と、あなたの中の何かが重なっている日。`;
    else if (a.aspect.symbol === '△') aspNote = `流れが自然につながりやすい日かも。`;
    else if (a.aspect.symbol === '☍') aspNote = `何かと何かが向き合っている、そんな感覚がある日。`;
    else if (a.aspect.symbol === '□') aspNote = `少しだけ摩擦を感じる場面があるかもしれない。`;
  }
  
  const titleOptions = [
    `${moonSign}の${vibe}一日`,
    `${moonSign}が包む、今日という時間`,
    `${moonSign}の風が吹いている`,
  ];
  ...
  text += phaseNote;
  text += ingressNote;
  text += `月は今日、${moonSign}にある。${vibe}空気が漂っている。\n`;
  text += `太陽は${sunSign}で、${moon.deg}度あたりの月の光が、今日のリズムをつくっているかも。\n`;
  if (aspNote) text += aspNote + '\n';
  text += '\n';
  text += `——— 今日、何かひとつだけ「後回しにしてたこと」に手をつけてみるとしたら、何がうかぶ？\n\n`;
  text += `素敵な1日を🍃`;
...
function generateMode2(dateStr) {
  ...
  // 月相
  if (todayPhase) {
    text += `◈ ${todayPhase.symbol}${todayPhase.type}（${todayPhase.time} JST）\n`;
  } else if (moonPhase) {
    text += `◈ 直近の${moonPhase.type}：${moonPhase.date} ${moonPhase.time}\n`;
  }
