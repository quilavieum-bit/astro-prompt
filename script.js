...
addData('2026-04-19', parsePDF('28a35'), parsePDF('13b0'), parsePDF('4a30'), parsePDF('23b15'), parsePDF('6a50'), parsePDF('17d21'), parsePDF('7a42'), parsePDF('29b36'), parsePDF('2a51'), parsePDF('5k26'));
...
function interpolate(abs0, abs1) {
  return interpolateAngle(abs0, abs1, 0.5);
}
...
function getPlanets(dateStr) {
  ...
  if (!d0) return null;
  if (!d1) return d0.map(v => v); // 翌日データなければそのまま
...
const PLANET_EN_NAMES = ['Sun','Moon','Mercury','Venus','Mars','Jupiter','Saturn','Uranus','Neptune','Pluto'];
...
// ============================================================
// 月相データ (国立天文台ベース 2026年・JST)
// ============================================================
const MOON_PHASES = [
  { date: '2026-04-06', time: '03:00', type: '新月', symbol: '🌑', desc: '牡羊座16°' },
  { date: '2026-04-13', time: '14:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-04-21', time: '05:00', type: '満月', symbol: '🌕', desc: '天秤座1°' },
  { date: '2026-04-29', time: '04:00', type: '下弦', symbol: '🌗', desc: '' },
  ...
  { date: '2026-06-04', time: '23:00', type: '新月', symbol: '🌑', desc: '双子座14°' },
  { date: '2026-06-11', time: '23:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-06-19', time: '09:00', type: '満月', symbol: '🌕', desc: '射手座28°' },
  { date: '2026-06-27', time: '03:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-07-04', time: '17:00', type: '新月', symbol: '🌑', desc: '蟹座12°' },
  { date: '2026-07-11', time: '20:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-07-19', time: '00:00', type: '満月', symbol: '🌕', desc: '山羊座27°' },
  { date: '2026-07-26', time: '14:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-08-03', time: '08:00', type: '新月', symbol: '🌑', desc: '獅子座11°' },
  { date: '2026-08-10', time: '15:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-08-17', time: '16:00', type: '満月', symbol: '🌕', desc: '水瓶座25°' },
  { date: '2026-08-25', time: '04:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-09-01', time: '20:00', type: '新月', symbol: '🌑', desc: '乙女座9°' },
  { date: '2026-09-09', time: '06:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-09-16', time: '07:00', type: '満月', symbol: '🌕', desc: '魚座23°' },
  { date: '2026-09-23', time: '22:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-10-01', time: '06:00', type: '新月', symbol: '🌑', desc: '天秤座8°' },
  { date: '2026-10-08', time: '21:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-10-15', time: '21:00', type: '満月', symbol: '🌕', desc: '牡羊座22°' },
  { date: '2026-10-23', time: '11:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-10-30', time: '13:00', type: '新月', symbol: '🌑', desc: '蠍座7°' },
  { date: '2026-11-07', time: '11:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-11-14', time: '10:00', type: '満月', symbol: '🌕', desc: '牡牛座22°' },
  { date: '2026-11-22', time: '03:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-11-29', time: '00:00', type: '新月', symbol: '🌑', desc: '射手座7°' },
  { date: '2026-12-07', time: '01:00', type: '上弦', symbol: '🌓', desc: '' },
  { date: '2026-12-14', time: '00:00', type: '満月', symbol: '🌕', desc: '双子座22°' },
  { date: '2026-12-21', time: '18:00', type: '下弦', symbol: '🌗', desc: '' },
  { date: '2026-12-28', time: '10:00', type: '新月', symbol: '🌑', desc: '山羊座7°' },
];
...
// 月相取得関数
// ============================================================
function getPhaseTimestamp(phase) {
  return new Date(`${phase.date}T${phase.time}:00+09:00`).getTime();
}

function getSelectionTimestamp(dateStr, timeStr = '12:00') {
  return new Date(`${dateStr}T${timeStr}:00+09:00`).getTime();
}

function getMoonPhaseInRange(startStr, endStr) {
  const start = getSelectionTimestamp(startStr, '00:00');
  const end = getSelectionTimestamp(endStr, '23:59');
  return MOON_PHASES
    .filter(mp => {
      const ts = getPhaseTimestamp(mp);
      return ts >= start && ts <= end;
    })
    .sort((a, b) => getPhaseTimestamp(a) - getPhaseTimestamp(b));
}
function getLatestMoonPhase(dateStr, timeStr = '12:00') {
  const selected = getSelectionTimestamp(dateStr, timeStr);
  const past = MOON_PHASES
    .filter(mp => getPhaseTimestamp(mp) <= selected)
    .sort((a, b) => getPhaseTimestamp(a) - getPhaseTimestamp(b));
  return past[past.length - 1] || null;
}
function getExactMoonPhaseAtSelection(dateStr, timeStr = '12:00') {
  const latest = getLatestMoonPhase(dateStr, timeStr);
  return latest && latest.date === dateStr ? latest : null;
}
...
function getJapaneseDateLabel(dateStr) {
  const d = new Date(`${dateStr}T12:00:00+09:00`);
  const weekdays = ['日','月','火','水','木','金','土'];
  return `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日（${weekdays[d.getDay()]}）`;
}

function formatMoonPhaseLine(phase, dateStr) {
  if (!phase) return '月相：データなし';
  const sameDay = phase.date === dateStr ? '' : '直近の';
  const desc = phase.desc ? ` ${phase.desc}` : '';
  return `月相：${sameDay}${phase.type}（${phase.date} ${phase.time} JST${desc}）`;
}

function formatTransitPositionLine(planets) {
  return planets.map((abs, i) => {
    return `${PLANET_SYMBOLS[i]}${PLANET_EN_NAMES[i]}:${formatPosStr(abs)}`;
  }).join(' / ');
}

function formatIngressLines(ingresses) {
  if (ingresses.length === 0) return 'なし';
  return ingresses.map(i => `${i.planet}：${i.prev}→${i.sign}`).join('\n');
}

function getAspectLabel(symbol) {
  const labels = {
    '☌': '合',
    '☍': 'オポジション',
    '□': 'スクエア',
...
function formatTransitAspect(i, j, aspect) {
  const firstName = i === 1 ? '' : PLANET_EN_NAMES[i];
  return `${PLANET_SYMBOLS[i]}${firstName}${aspect.symbol}${PLANET_SYMBOLS[j]}${PLANET_EN_NAMES[j]}(${getAspectLabel(aspect.symbol)},orb${aspect.diff.toFixed(1)}°)`;
}
...
function getTransitAspects(planets) {
  ...
  for (let i = 0; i < planets.length; i++) {
    for (let j = i + 1; j < planets.length; j++) {
      const involvesMoon = i === 1 || j === 1;
      const aspect = checkAspect(planets[i], planets[j], involvesMoon);
      if (!aspect) continue;
      const item = { i, j, aspect, text: formatTransitAspect(i, j, aspect) };
      if (involvesMoon) moonAspects.push(item);
      else planetAspects.push(item);
    }
  }
  const byTightness = (a, b) => a.aspect.diff - b.aspect.diff;
  return {
    moonAspects: moonAspects.sort(byTightness),
    planetAspects: planetAspects.sort(byTightness),
  };
}

function formatAspectSection(items, limit) {
  if (items.length === 0) return 'なし';
  return items.slice(0, limit).map(item => item.text).join('\n');
}

function generateShortReading(moon, moonAspects, planetAspects, phase) {
  const moonMood = getMoonMoodPhrase(moon.sign);
  const mainMoon = moonAspects[0];
  const mainPlanet = planetAspects[0];
  const flow = mainMoon ? getAspectMoodLine([{ aspect: mainMoon.aspect }]) : moonMood.line;
  const focus = mainPlanet ? getAspectMoodLine([{ aspect: mainPlanet.aspect }]) : getPhaseMoodLine(phase);
  return `300字以内で：\n・今日のエネルギー：${moon.sign}の月が軸。${flow}。${focus}。\n・おすすめ：感じたことを表現する／予定を詰めすぎない／小さく整える。\n・注意点：勢いだけで決めず、違和感を一度見つめる。\n・あわい言葉は、下の短い言葉をもとに余白を残して書く。`;
}

function getMoonMoodPhrase(sign) {
  const moods = {
    '牡羊座': { title: '風が変わる日', line: '何かが急に動き出すかも', question: '今日何を始めたい？', verb: '始めて' },
    '牡牛座': { title: '深く息をする日', line: 'ゆっくり触れたものが力になるかも', question: '今日何を大切にしたい？', verb: '味わって' },
    '双子座': { title: '言葉がほどける日', line: 'ふとした会話から道がひらくかも', question: '今日誰に伝えたい？', verb: '伝えて' },
    '蟹座': { title: '心の奥に戻る日', line: '安心できる場所が見つかるかも', question: '今日どこで休みたい？', verb: '満たして' },
    '獅子座': { title: '光を思い出す日', line: '小さな表現が誰かに届くかも', question: '今日何を表現したい？', verb: '輝いて' },
    '乙女座': { title: '整えて軽くなる日', line: 'ひとつ片づけるだけで呼吸が楽になるかも', question: '今日何を整えたい？', verb: '整えて' },
    '天秤座': { title: 'やわらかく結び直す日', line: '距離感を少し変えると楽になるかも', question: '今日誰と心地よくいたい？', verb: '結んで' },
    '蠍座': { title: '静かに深まる日', line: '見ないふりをしていた気持ちに触れるかも', question: '今日何を手放したい？', verb: '深めて' },
    '射手座': { title: '遠くを見たくなる日', line: 'いつもより広い場所へ心が向かうかも', question: '今日どこへ向かいたい？', verb: '広げて' },
    '山羊座': { title: '形にしていく日', line: '小さな積み重ねが未来を支えるかも', question: '今日何を一歩進めたい？', verb: '築いて' },
    '水瓶座': { title: '新しい風を入れる日', line: '違う見方をすると急に軽くなるかも', question: '今日何を変えてみたい？', verb: 'ひらいて' },
    '魚座': { title: '境目がほどける日', line: 'やさしさの中で答えが浮かぶかも', question: '今日何をゆるめたい？', verb: 'ゆるめて' },
  };
  return moods[sign] || moods['牡牛座'];
}

function getAspectMoodLine(aspects) {
  const tight = aspects.find(a => a.aspect.tight) || aspects[0];
  if (!tight) return 'いつもの道にも小さな合図があるかも';
  if (tight.aspect.symbol === '☌') return '重なって見えてくるものがあるかも';
  if (tight.aspect.symbol === '△') return '流れにまかせると進みやすいかも';
  if (tight.aspect.symbol === '⚹') return '小さなきっかけが扉になるかも';
  if (tight.aspect.symbol === '□') return '少し立ち止まることで見えてくるかも';
  if (tight.aspect.symbol === '☍') return '向き合うことでほどけるものがあるかも';
  return '心の向きが少し変わっていくかも';
}

function getPhaseMoodLine(phase) {
  if (!phase) return 'まだ言葉にならない予感があるかも';
  if (phase.type === '新月') return 'まだ形のない願いが静かに芽を出すかも';
  if (phase.type === '満月') return '満ちたものをそっと受け取れるかも';
  if (phase.type === '上弦') return '迷いながらも一歩進めるかも';
  if (phase.type === '下弦') return '余分なものを少し手放せるかも';
  return '心の波に合わせて進めるかも';
}
...
function generateMode1(dateStr) {
  ...
  const moonPhase = getLatestMoonPhase(dateStr);
  const todayPhase = getExactMoonPhaseAtSelection(dateStr);
  const phase = todayPhase || moonPhase;
  const { moonAspects, planetAspects } = getTransitAspects(planets);
  const mood = getMoonMoodPhrase(moon.sign);
  const ingressLine = ingresses.length > 0 ? '空気が少し入れ替わるかも' : null;
  const bodyLines = [
    mood.line,
    ingressLine || getPhaseMoodLine(phase),
    getAspectMoodLine(moonAspects.concat(planetAspects)),
  ];

  let text = `【${getJapaneseDateLabel(dateStr)}】${formatMoonPhaseLine(phase, dateStr)}\n\n`;
  text += `◆天体位置\n`;
  text += `${formatTransitPositionLine(planets)}\n\n`;
  text += `◆イングレス\n`;
  text += `${formatIngressLines(ingresses)}\n\n`;
  text += `◆月のアスペクト（トランジット同士）\n`;
  text += `${formatAspectSection(moonAspects, 6)}\n\n`;
  text += `◆天体間アスペクト\n`;
  text += `${formatAspectSection(planetAspects, 8)}\n\n`;
  text += `${generateShortReading(moon, moonAspects, planetAspects, phase)}\n\n`;
  text += `◆あわい言葉\n`;
  text += `今日のあわい言葉${getDateLabel(dateStr)}\n\n`;
  text += `${mood.title}\n\n`;
  bodyLines.forEach(line => { text += `${line}\n\n`; });
  text += `${mood.question}\n\n`;
  text += `素敵な1日を🌱`;
...
function generateMode2(dateStr) {
  ...
  const moon = formatPos(planets[1]);
  const sun = formatPos(planets[0]);
  
  const tight = aspects.filter(a => a.aspect.tight);
  const important = aspects.slice(0, 5);
  
  let text = `【${dateStr} 個人リーディング】\n\n`;
  
  // 月相
  if (moonPhase) {
    const phaseLabel = todayPhase ? todayPhase.type : `直近の${moonPhase.type}`;
    text += `◈ ${moonPhase.symbol}${phaseLabel}：${moonPhase.date} ${moonPhase.time} JST\n`;
  }
  
  // イングレス
  if (ingresses.length > 0) {
    text += `◈ イングレス：${ingresses.map(i => `${i.planet}→${i.sign}`).join('、')}\n`;
  }
  
  text += `\n▸ 今日の特徴\n`;
  text += `月${moon.sign}${moon.deg}° 太陽${sun.sign}${sun.deg}° の空。`;
  
  if (tight.length > 0) {
    text += `\n特に注目すべきは `;
    tight.slice(0,2).forEach(a => {
      text += `T${a.transit}${a.aspect.symbol}${a.natal}（${a.aspect.diff.toFixed(1)}°）`;
    });
    text += `。強い共鳴が起きている。`;
  }
  ...
  text += `\n\n▸ 行動のヒント\n`;
  // アスペクトから行動提案
  important.slice(0,3).forEach(a => {
    const actions = aspectToAction(a);
    if (actions) text += `・${actions}\n`;
  });
  
  text += `\n▸ 注意点\n`;
  const squares = aspects.filter(a => a.aspect.symbol === '□' || a.aspect.symbol === '☍').slice(0,2);
  if (squares.length > 0) {
    squares.forEach(a => {
      text += `・${a.transit}×${a.natal}：${squareWarning(a)}\n`;
    });
  } else {
    text += `・大きな緊張なし。自然体で過ごして。\n`;
  }
  
  text += `\n▸ メッセージ\n`;
  text += generatePersonalMessage(aspects, moon, planets);
...
function getWeekFinalAction(sign, phases, ingresses) {
  if (phases.some(p => p.type === '新月')) return '芽を育てる';
  if (phases.some(p => p.type === '満月')) return '本音を受け取る';
  if (phases.some(p => p.type === '下弦')) return '余白をつくる';
  if (phases.some(p => p.type === '上弦')) return '一歩進める';
  if (ingresses.length > 0) return '流れを変える';
  const actions = {
    '牡羊座': '動き出す',
    '牡牛座': '土台を整える',
    '双子座': '言葉にする',
  ...
  return actions[sign] || '整える';
}

function generateAwaiWeekWord(start, weekData, phases, ingresses) {
  const weekNo = getISOWeekNumber(start);
  const startSign = weekData[0] ? SIGNS[Math.floor(weekData[0].planets[1] / 30)] : '牡牛座';
  const mid = weekData[Math.floor(weekData.length / 2)] || weekData[0];
  const end = weekData[weekData.length - 1] || mid;
  const midSign = mid ? SIGNS[Math.floor(mid.planets[1] / 30)] : startSign;
  const endSign = end ? SIGNS[Math.floor(end.planets[1] / 30)] : midSign;
  const firstVerb = getMoonMoodPhrase(startSign).verb;
  const midVerb = getMoonMoodPhrase(midSign).verb;
  const finalAction = getWeekFinalAction(endSign, phases, ingresses);
  return `◆あわいの週言葉\nWeek ${weekNo}\n${firstVerb}、${midVerb}、${finalAction}週`;
}
...
function generateMode3(dateStr) {
  ...
  text += `${generateAwaiWeekWord(start, weekData, phases, ingresses)}\n\n`;
